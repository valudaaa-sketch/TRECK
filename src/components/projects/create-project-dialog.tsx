    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px] bg-[#0c0c0e]/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-[0_0_40px_rgba(0,0,0,0.5)] text-white">
        <form action={onSubmit}>
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setOpen(false)} className="bg-white/5 border-white/10 text-white hover:bg-white/10 hover:text-white rounded-full">
              Cancel
            </Button>
            <Button type="submit" disabled={isLoading} className="bg-white text-black hover:bg-gray-100 rounded-full font-semibold shadow-[0_0_15px_rgba(255,255,255,0.1)]">
